...
def prepare_data(df, params):
    ...
    for col_name in cols_to_encode[1:]:
        df[f"sim_{ref_col}_{col_name}"] = df.apply(
            lambda row: _compute_similarity(
                row[f"{ref_col}_encoded"], row[f"{col_name}_encoded"]
            ),
            axis=1,
        )
    ...
    df.loc[:, "num_comma_all_queries"] = df.loc[:, "all_queries"].apply(
        lambda qlist: [q.strip().count(",") for q in qlist]
    )
    ...
    df.loc[:, "num_words_all_queries"] = df.loc[:, "all_queries"].apply(
        lambda qlist: [
            len(q.strip().replace(",", " ").split(" "))
...
def split_data(df, params):
    ...
    df_success_addr = df_success[df_success["poi_proba"] < (1 - poi_proba_threshold)]
...
def reverse_geocode(df, params):
    ...
    rev_geocode = RateLimiter(
        geolocator.reverse, min_delay_seconds=params["min_delay_seconds"]
    )

    df = df[df["location"].apply(lambda x: len(str(x)) > 10)]

    df["location_dict"] = df["location"].apply(lambda x: literal_eval(x))
    df["location_coordinates"] = df["location_dict"].apply(
        lambda d: str(d["lat"]) + "," + str(d["lon"])
    )
...
def filter_rev_geocode(df, params):
    ...
    df[col_geo] = df["reverse_geocode"].apply(lambda l: str(l).split(",")[0].strip())
    ...
    count = len(df)
    log.info(
        f"Count for FAIL POIs after removing reverse geocoding starting with number {count}"
    )
    ...
    df.loc[:, f"sim_query_{col_geo}"] = df.apply(
        lambda row: _compute_similarity(
            row[f"query_encoded"], row[f"{col_geo}_encoded"]
        ),
        axis=1,
    )

    df.loc[:, "country_query"] = (
        df["reverse_geocode"].apply(lambda x: x.split(",")[-1]).str.strip()
    )
    ...
    osm_better = df[df["sim_query_reverse_location"] > df["sim_query_poiName"]]

    count = len(osm_better)
    log.info(f"Count for better reverse than poiName is {count}")

    return {"all": df, "osm_better": osm_better}
...
def get_bing_result(df, params):
    ...
    client = _create_bing_client(bing_key)

    df["user_lat"] = df["user_latlon"].apply(lambda x: x.split(",")[0])
    df["user_lon"] = df["user_latlon"].apply(lambda x: x.split(",")[0])

    df["query_bing"] = df.apply(
        lambda row: search(
            client, row["query"], latitude=row["user_lat"], longitude=row["user_lon"]
        ),
        axis=1,
    )
    return [df]
...
def search(client, query: str, latitude: str = None, longitude: str = None):
...
def _encode_string(df, col_name, transformer):
    df.loc[:, f"{col_name}_encoded"] = df.loc[:, f"{col_name}"].apply(
        lambda sentence: transformer.encode(str(sentence))
